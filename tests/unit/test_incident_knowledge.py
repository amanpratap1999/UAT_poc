...
def test_priority_matrix_calculation() -> None:
    ...
    assert (
        IncidentBusinessRules.calculate_priority(Impact.HIGH, Urgency.HIGH)
        == IncidentPriority.CRITICAL
    )

    # High x Medium = High (2)
    assert (
        IncidentBusinessRules.calculate_priority(Impact.HIGH, Urgency.MEDIUM)
        == IncidentPriority.HIGH
    )

    # Low x Low = Planning (5)
    assert (
        IncidentBusinessRules.calculate_priority(Impact.LOW, Urgency.LOW)
...
def test_incident_lifecycle_transitions() -> None:
    ...
    assert (
        IncidentLifecycle.is_valid_transition(IncidentState.NEW, IncidentState.IN_PROGRESS) is True
    )
    ...
    assert (
        IncidentLifecycle.is_valid_transition(IncidentState.IN_PROGRESS, IncidentState.RESOLVED)
        is True
    )
